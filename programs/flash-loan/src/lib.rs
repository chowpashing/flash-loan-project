...
declare_id!("GfUtMsTY9MmV9oEN65GPAzy3pVmCq3yiB1d4Ub7WSPZ4");
...
pub mod flash_loan_program {
    ...
    pub fn atomic_flash_loan_with_arbitrage(
        ...
        // 1. 先更新池子状态 - 防止重入攻击 (CEI模式: Check-Effects-Interactions)
        ...
        **ctx.accounts.borrower.to_account_info().try_borrow_mut_lamports()? += amount;
        ...
        let actual_profit = {
            ...
            arbitrage_bot::cpi::execute_arbitrage_atomic(cpi_ctx, amount, min_expected_profit)?.get()
        };
        
        msg!("✅ 套利完成，获得利润: {} lamports", actual_profit);
...
pub struct AtomicFlashLoanWithArbitrage<'info> {
    ...
    #[account(
        init,
        payer = borrower,
        seeds = [b"transaction_record", borrower.key().as_ref(), &Clock::get()?.unix_timestamp.to_le_bytes()],
        bump,
